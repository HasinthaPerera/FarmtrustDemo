        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCrops.length > 0 ? (
            filteredCrops.map(crop => (
              <div key={crop.id} className="glass-card hover:shadow-2xl hover:scale-[1.02] transition-transform">
              ...
              orders.slice(0, 8).map(order => (
                <div key={order.id} className="card">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-bold text-slate-900">{order.crop?.name || 'Crop'}</h3>
                    <span className={`badge ${order.paymentStatus === 'paid' ? 'badge-success' : 'badge-warning'}`}>
                      {order.paymentStatus}
                    </span>
                  </div>
                  <p className="text-slate-600">Qty: {order.quantity} kg</p>
                  <p className="text-slate-600">Total: ${Number(order.totalAmount).toFixed(2)}</p>
                  <p className="text-slate-600">Method: {order.paymentMethod}</p>
                  {order.paymentReference && (
                    <p className="text-slate-600">Ref: {order.paymentReference}</p>
                  )}
                </div>
              ))
            ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/55 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-slate-900">Payment Window</h3>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <p className="font-semibold text-slate-900">Crop: {selectedCrop.name}</p>
                <p className="text-slate-600">Unit price: ${Number(selectedCrop.price).toFixed(2)} / kg</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="input-field"
                  disabled={paymentLoading || !!orderIntent}
                />
              </div>
              ...
              {!orderIntent ? (
                ...
                <div className="space-y-3">
                  <div className="rounded-xl border border-slate-200 p-4 bg-slate-50">
                    <p className="font-semibold text-slate-900">Order #{orderIntent.orderId} is ready</p>
                    <p className="text-slate-600">Click an option below to simulate payment result.</p>
                  </div>
                  <button
                    type="button"
                    className="btn-primary w-full"
